...
export type PostMortemData = {
  ...
  incidentStatusTone?: "success" | "warning" | "error" | "neutral";
  incidentId?: string;
  incidentFooterText?: string;
...
const DEFAULT_REPORT_FIELDS: ReportField[] = [
...
const DEFAULT_IMPROVEMENTS: string[] = [
...
export const PostMortem: React.FC<PostMortemProps> = ({ opacity, data }) => {
  ...
  const phaseLabel = data?.phaseLabel ?? "Final Phase · SOC Operations";
  const title = data?.title ?? "Post-Incident Review & Lessons Learned";
  const incidentStatusLabel = data?.incidentStatusLabel ?? "NEUTRALIZED";
  const incidentStatusTone = data?.incidentStatusTone ?? "success";
  const incidentFooterText = data?.incidentFooterText ?? "INC-2024-047 · Closed";
  const timelineStages = data?.timelineStages ?? DEFAULT_TIMELINE_STAGES;
  const reportFields = data?.reportFields ?? DEFAULT_REPORT_FIELDS;
  const lessons = data?.lessons ?? DEFAULT_LESSONS;
  const improvements = data?.improvements ?? DEFAULT_IMPROVEMENTS;

  const statusToneClasses: Record<
    NonNullable<PostMortemData["incidentStatusTone"]>,
  ...
  const tone = statusToneClasses[incidentStatusTone];
  ...
  return (
    ...
    >
      ...
      <div
        ref={contentRef}
        className="relative z-10 w-full max-w-[1100px] px-4 sm:px-6 md:px-8 opacity-0 flex flex-col gap-5 md:gap-6"
      >

        {/* ── Title ── */}
        <div className="text-center">
          <p className="font-mono text-[9px] sm:text-[10px] uppercase tracking-[0.6em] text-sky-400/60 mb-1">
            {phaseLabel}
          </p>
          <h2
            className="font-mono text-xl sm:text-2xl md:text-3xl font-black uppercase tracking-widest text-white"
        ...
        <div className="pm-col opacity-0 rounded-xl border border-sky-500/15 bg-slate-950/50 px-4 sm:px-6 py-4 backdrop-blur-sm relative overflow-hidden" style={{ boxShadow: "0 0 50px rgba(14,165,233,0.04)" }}>
          <p className="font-mono text-[8px] sm:text-[9px] uppercase tracking-[0.45em] text-sky-400/60 mb-4">Attack Lifecycle</p>
          <div className="flex items-center justify-between gap-4 sm:gap-6 relative overflow-x-auto pr-2">
            {/* connector line */}
            ...
            {timelineStages.map((stage, i) => (
              <div key={stage.label + i} className="pm-node opacity-0 flex flex-col items-center gap-2 relative z-10" style={{ minWidth: "clamp(56px, 7.5vw, 80px)" }}>
                <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          ...
          <div className="pm-col opacity-0 col-span-1 rounded-xl border border-sky-500/20 bg-slate-950/60 p-4 sm:p-5 backdrop-blur-md relative" style={{ boxShadow: "0 0 40px rgba(14,165,233,0.05)" }}>
            ...
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-white/5 gap-2">
              <p className="font-mono text-[8px] sm:text-[9px] uppercase tracking-[0.4em] text-sky-400/70">📋 Incident Summary</p>
              <span
                className={`font-mono text-[8px] sm:text-[9px] font-bold px-2 py-0.5 rounded-sm border ${tone.bg} ${tone.border} ${tone.text}`}
              >
                {incidentStatusLabel}
              </span>
            </div>
            <div className="space-y-2.5">
              {reportFields.map((row, i) => (
                <div key={row.label + i} className="pm-row opacity-0 flex justify-between items-baseline gap-2">
                  <span className="font-mono text-[8px] sm:text-[9px] uppercase tracking-wider text-slate-500 shrink-0">{row.label}</span>
                  <span className="font-mono text-[10px] sm:text-[11px] font-bold text-sky-200 text-right">{row.value}</span>
                </div>
              ))}
            </div>
            <p className="font-mono text-[8px] sm:text-[9px] uppercase text-slate-600 tracking-widest text-center mt-4">
              {incidentFooterText}
            </p>
          </div>

          {/* CENTER: Lessons Learned text */}
          <div className="pm-col opacity-0 col-span-1 rounded-xl border border-amber-500/20 bg-slate-950/60 p-4 sm:p-5 backdrop-blur-md relative flex flex-col" style={{ boxShadow: "0 0 40px rgba(245,158,11,0.04)" }}>
            ...
            <div className="flex-1 space-y-2.5">
              {lessons.map((point, i) => (
                <div key={point + i} className="pm-row opacity-0 flex items-start gap-2.5">
          ...
          <div className="pm-col opacity-0 col-span-1 rounded-xl border border-emerald-500/20 bg-slate-950/60 p-4 sm:p-5 backdrop-blur-md relative" style={{ boxShadow: "0 0 40px rgba(52,211,153,0.05)" }}>
            ...
            <ul className="space-y-3">
              {improvements.map((item, i) => (
                <li key={item + i} className="pm-row opacity-0 flex items-start gap-2.5">
