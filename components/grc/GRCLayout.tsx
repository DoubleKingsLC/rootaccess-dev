...
import { GRCIntroOverlay } from "./GRCIntroOverlay";
import { GRCOutroScene } from "./GRCOutroScene";
import { MorningRitualsScene } from "./MorningRitualsScene";
import { ComplianceFrameworksScene } from "./ComplianceFrameworksScene";
import { RiskAssessmentScene } from "./RiskAssessmentScene";
import { CollaborationScene } from "./CollaborationScene";
import { DocumentationScene } from "./DocumentationScene";
import { AuditIncidentScene } from "./AuditIncidentScene";
...
// ── Background packets ─────────────────────────────────────────────────────────
const PACKETS = [
  { left: "0%",    top: "18%",  anim: "packet-h",     dur: 14, delay: 0   },
  { left: "0%",    top: "55%",  anim: "packet-h",     dur: 11, delay: 2.5 },
  { left: "100%",  top: "30%",  anim: "packet-h-rev", dur: 13, delay: 1   },
  { left: "100%",  top: "70%",  anim: "packet-h-rev", dur: 10, delay: 3   },
  { left: "25%",   top: "0%",   anim: "packet-v",     dur: 12, delay: 0.5 },
  { left: "70%",   top: "0%",   anim: "packet-v",     dur: 15, delay: 2   },
  { left: "45%",   top: "100%", anim: "packet-v-rev", dur: 11, delay: 1   },
  { left: "82%",   top: "100%", anim: "packet-v-rev", dur: 13, delay: 3.5 },
];

/** Scroll progress (0–1) where intro overlay is cleared; autoplay uses 4× speed until then (no instant jump). */
...
export const GRCLayout: React.FC = () => {
  ...
  if (showRecordedWorkflow) {
    return <RoadmapWorkflowMobileWalkthrough slug="grc" />;
  }
    ...
    <section ref={scrollSectionRef} className="relative h-[4000vh] w-screen" style={{ background: "#020f14" }}>
      ...
      >
        <div
          ...
          style={{
            backgroundImage: [
              "radial-gradient(ellipse at 50% 40%, rgba(20,184,166,0.05) 0%, transparent 65%)",
              "radial-gradient(circle at center, transparent 0%, rgba(2,15,20,0.92) 100%)",
              "linear-gradient(to right, rgba(15,30,32,0.6) 1px, transparent 1px)",
              "linear-gradient(to bottom, rgba(15,30,32,0.6) 1px, transparent 1px)",
            ].join(", "),
        ...
        <div ref={stageRef} className="absolute inset-0 z-10">
          <MorningRitualsScene progress={progress} />
          <ComplianceFrameworksScene progress={progress} />
          <RiskAssessmentScene progress={progress} />
          <CollaborationScene progress={progress} />
          <DocumentationScene progress={progress} />
          <AuditIncidentScene progress={progress} />
          <ReflectionScene progress={progress} />
          <GRCOutroScene progress={progress} />
        </div>
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: isActive ? `0 0 10px ${TEAL_GLOW}` : "none",
                      }}
                    />
                    <p
        ...
        >
          <div
            ...
            onMouseMove={handleSpeedInteraction}
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                boxShadow: isSpeedControlOpen ? `0 0 15px ${TEAL_GLOW}` : "none",
              }}
            >
              <span className="font-mono text-[10px] font-bold" style={{ color: TEAL }}>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: isAutoScrolling ? `0 0 20px ${TEAL_GLOW}` : "none",
            }}
          >
            {isAutoScrolling ? (
